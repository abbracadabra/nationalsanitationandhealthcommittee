...
if __name__=='__main__':
    ...
    for x, lb in getRealData():
        ls, _, ac = sess.run([loss, op, acc], feed_dict={images: np.float32(x), label: lb})
        print(ls, ac)
        if int(time.time()%50)==0:
            val_loss, val_acc = sess.run([loss,acc], feed_dict={images: np.float32(val_x), label: val_label})
            val_history+=[val_acc]
